...
fileName1="eu-004.pdf-01.jpg"
image = cv2.imread(cwd+"//images//"+fileName1)
image1 = cv2.imread(cwd+"/images//"+fileName1)
print("\n\t shape=",image.shape)
...
def lineSegmentation(fileName1,df1):
    ...
    # print("\n\t is file=",os.path.isfile(cwd+"//images//" + fileName1))
    # print("\n\t shape=",image.shape)
    ...
    def getWords(image11,curtLineNo,currentCor,countWords,wordCorLine,wordIndx,wordLine,cor,cor1,wordFeatureIndx):
        ...
        #hello = annot1["x1"][0]
        #print("\n\t annot1:\n\t",[annot1["x1"][0],annot1["y1"][0],annot1["x2"][0],annot1["y2"][0]])
    ...
    def initialize(cor,cor1):

        cor.loc[wordFeatureIndx, "0"] = [0, 0, 0, 0]
        cor1.loc[wordFeatureIndx, "0"] = [0, 0, 0, 0]

        cor.loc[wordFeatureIndx, "1"] = [0, 0, 0, 0]
        cor1.loc[wordFeatureIndx, "1"] = [0, 0, 0, 0]

        cor.loc[wordFeatureIndx, "2"] = [0, 0, 0, 0]
        cor1.loc[wordFeatureIndx, "2"] = [0, 0, 0, 0]

        cor.loc[wordFeatureIndx, "3"] = [0, 0, 0, 0]
        cor1.loc[wordFeatureIndx, "3"] = [0, 0, 0, 0]
        cor.loc[wordFeatureIndx, "4"] = [0, 0, 0, 0]
        cor1.loc[wordFeatureIndx, "4"] = [0, 0, 0, 0]
        ...
        cor1.loc[wordFeatureIndx, "6"] = [0, 0, 0, 0]

        cor.loc[wordFeatureIndx, "7"] = [0, 0, 0, 0]
        cor1.loc[wordFeatureIndx, "7"] = [0, 0, 0, 0]

        cor.loc[wordFeatureIndx, "8"] = [0, 0, 0, 0]
        cor1.loc[wordFeatureIndx, "8"] = [0, 0, 0, 0]

        cor.loc[wordFeatureIndx, "table"] = 0
        cor1.loc[wordFeatureIndx, "table"] = 0
    ...
    cv2.imwrite(cwd+"//featureImage//"+fileName1+".png",image)
...
for no,content in df.iterrows():

    #print("\n\t conte\n",content)
    fileName1=content.loc["fileName"]
    print("\n\t no:",no,"\t fileName1:",fileName1)

    try:

        lineSegmentation(fileName1,df1)
    except Exception as e:
        pass
