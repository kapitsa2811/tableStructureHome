...
def lineSegmentation(df1,fileName1,cor,cor1):
    ...
    totWordProcess = totWordProcess + len(tempWordCord)
    delmePath="/home/kapitsa/PycharmProjects/objectLocalization/wsl/tableStructure/delMe2//"
    ...
    cor,cor1,allwords=cordinateProcessing(fileName1,image,wordCorLine,wordLine,cor,cor1)

    return cor,cor1,totWordProcess,allwords
    # except Exception as e:
...
for fileIndx,fileName in enumerate(os.listdir(imagePath)):
    ...
    cor,cor1,totWordProcess,allwords=lineSegmentation(df1,fileName,cor,cor1)
    newCorShape=cor.shape
    print("\n\t 2.oldCorShape=",oldCorShape,"\t newCorShape=",newCorShape)
    print("\n\t lines adde:",newCorShape[0]-oldCorShape[0])
    print("\n\t df1 shape=",df1.shape)
    print("\n\t tempWordCord=",totWordProcess,"\t allwords=",allwords)
