...
def drawRect(cor,image):
    cv2.rectangle(image,(cor[0],cor[1]),(cor[2],cor[3]),(0,0,255),2)
    return image
...
def writeImageCC1(OutImage,tempCopy,ii,multi):
    ''''
    temporary change
    '''
    ...
    #print "\n\t OutImage+str(ii)+.jpg=",str(ii)+".jpg"
    #nameImage=str(ii).split("/")[-1]
    compID=str(ii)+"_"+str(multi) #str(ii).split("_")[1]
    #print("nameImage=",compID+nameImage)#+"_"+str(ii)+".jpg")
    nameImage=compID+"_.jpg" #+nameImage
    print("\n\t nameImage=",nameImage)
...
def show(name,img):
    ...
    try:
        ...
        cv2.imshow(name,img)
        cv2.moveWindow(name,500,300)
        cv2.waitKey()
        #cv2.destroyAllWindows()
    except Exception as e:
        print("\n\t exception in show e",e)
...
def displayAndSaveComponents(nm,original,labels,selComp,stats,ii,mediumSize,cc_threshold):
    imageFlag=0
    ...
    multi=dx*dy
    mediumSize.append(multi)

    if multi<cc_threshold:
        imageFlag=1
        #drawContour2(tempCopy,ii)
        crop=255-tempCopy[y:(y+dy),x:(x+dx)]
        #writeImageCC(outPath,crop,str(ii)+"_"+str([x,y,(x+dx),(y+dy)])+"_"+nm)
        ...
        #writeImageCC1(".//cc//delMe//",original,ii,multi)


        print("\n\t y:",y,"\t y+dy",(y+dy),"\t x:",x,"\t (x+dx):",(x+dx))
        print("\n\t dy:",dy,"\t dx:",dx)
        print("\n\t crop:",crop.shape)

        try:
             selComp[y:(y+dy),x:(x+dx),0]=crop
             selComp[y:(y+dy),x:(x+dx),1]=crop
             selComp[y:(y+dy),x:(x+dx),2]=crop
             #show("selComp", selComp)
        except Exception as e:
             print("\n\t exception in croping",e)
             exc_type, exc_obj, exc_tb = sys.exc_info()
             fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
             print("\n\t LINE NO:", exc_tb.tb_lineno)

        print("\n\t ")
        #writeImageCC(".//cc//",tempCopy,ii)
        cv2.imwrite("./cc//"+str(ii)+".jpg",tempCopy)
        #show(str(ii), tempCopy)
        #show(str(ii),crop)
        #input("check cc")
    else:
        original[tempCopy==ii]=255
        aTemp=np.where(tempCopy==ii)
        print("\n\t aTemp:",aTemp[0])
        print("\n\t changing len=",len(aTemp))
        cv2.imwrite("./cc//"+str(ii)+".jpg",original)
        #input("changing")

    return original,mediumSize,selComp
...
def readImages(dataPath):
    ...
    for indx,nm in enumerate(files):
        ...
        #print "\n\t shape=>", img1.shape
        #selComp=np.zeros(shape=(img1.shape[0],img1.shape[1],3))
        selComp=np.ones(shape=(img1.shape[0],img1.shape[1],3))*255
        ...
        for ii in np.unique(labels):
            ...
            original,mediumSize,selComp=displayAndSaveComponents(nm,original,labels,selComp,stats,ii,mediumSize,cc_threshold)
        #writeImageCC1(outPath+"//delMe//",original,nm[:-4])
        #writeImageCC1(".//cc//delMe//", original, ii, nm[:-4])


        nm1=nm.split("/")[-1]
        print("\n\t nm1=",nm1)
        cv2.imwrite(".//cc//delMe//" + nm1, original)
        cv2.imwrite("./filterComp//" + nm1, selComp)
        print("\n\t median:",statistics.median(mediumSize))
        print("\n\t max component:",max(mediumSize))
        #writeImageCC(outPath,crop,str(ii)+"_"+str([x,y,(x+dx),(y+dy)])+"_"+nm)
        #print("\n\t average:",statistics.average(mediumSize))

        # except Exception as e:
        #     print ("\n\t fileName=",nm)
        #     print("\n\t Exception in readImages")
        #     exc_type, exc_obj, exc_tb = sys.exc_info()
        #     fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        #     print("\n\t LINE NO:", exc_tb.tb_lineno)
        #     exceptionCount+=1
